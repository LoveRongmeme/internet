...
* **差错检测**:数据在传输过程中可能会出现差错，1可能变成0,0也可能变成1，这样的差错叫**比特差错**，也是传输差错的一种；传输错误的比特占总数的比例叫**误码率**---提高信噪比可以降低误码率；目前数据链路层广泛使用**循环冗余检验CRC(Cyclic Redundancy Check)** 检测技术；如果要在数据链路层实现差错检测，必须要把数据封装成帧，每一帧加上冗余码(FCS),逐帧发送及检错；数据链路层若仅仅使用**CRC**检错技术，则只能实现帧的**无差错接受**---**凡是接收端数据链路层接受的帧均是无差错的**;数据传输还可能出现另一种差错:**帧丢失**|**帧重复**|**帧失序**，**因此使用CRC只能实现无比特差错传输，并不能提供可靠传输**;<br>

**ppp协议**(point-to-point protocol):用户计算机和ISP进行通信时所用的数据链路层协议;**ppp协议**应满足的需求:
* **简单**:提供不可靠传输服务，无需检错、无需序号、无需流量控制；只需要通过**CRC**检测，就收下这个帧，否则就丢弃这个帧；
* **封装成帧**:必须指定特殊字符为**帧定界符**;
* **透明性**:必须有有效措施来应对数据中出现的**帧定界符**;
* **多中网络层协议**:必须在**同一条物理链路上支持多种网络层协议(比如IP、IPX等)；
* **多种类型链路**:必须能在多种类型的链路上传输(串行|并行、低速|高速、同步|异步等)；
* **差错检测**，**及时检测连接状态**，**最大传输单元**,**网络层地址协商**(必须有一种协商机制让通信双方能够知道彼此的网络层地址),**数据压缩协商**;<br>

**PPP协议的组成**:三部分
* 一个将IP数据报封装到串行链路的方法;
* 一个用来建立、配置和测试数据链路连接的**链路控制协议(Link Control Protocol)**;
* 一套**网络控制协议(Network Control Protocol)**,能够支持多种网络层协议；<br>

**使用广播信道的数据链路层**<br>
局域网的主要特点:**网络为一个单位所有，且地理范围和站点数目均有限**;<br>
局域网按拓扑结构可以分为三类:**星型网**(目前广泛使用的),**环形网**,**总线网**(以传统以太网最为著名);<br>
**共享信道**技术有两种:
* **静态划分信道**:频分复用、时分复用、波分复用、码分复用等，不适合局域网使用；
* **动态媒体接入控制(多点接入)**:又分为两种:
   * **随机接入**:用户可随机发送信息---可能会产生**碰撞**;---是局域网主要方式
   * **受控接入**:用的较少
<br>
   
为了是数据链路层能够更好的适应多种局域网标准，局域网的数据链路层被分成两层:**逻辑链路控制层LLC(Logical Link Control )**、**媒体接入控制MAC(Medium Access Control)** (与接入到传输媒体有关的都放在这一层)；<br>
**适配器**:计算机与外界局域网的连接是通过通信适配器实现的，说白了就是**网卡**,现在一般都集成在计算机主板上，不再单独使用；适配器上有处理器和储存器(RAM|ROM);计算机与适配器之间使用并行传输，适配器与局域网之间使用串行传输，因此适配器要能实现传输方式的转化；计算机的硬件地址就储存在适配器的储存器(ROM)中，而计算机的软件地址(IP地址)则储存在计算的储存器中；<br>
给每个适配器分配地址，给传输的帧附上目的地址，这样可以实现一对多或者一对一传输数据，因此这种传输也是不可靠的传输---尽最大努力的传输；同时为了防止总线上同时有别的站点在发送数据而产生碰撞，就采用了CSMA/CD协议---**载波监听多点接入/碰撞检测**;不管在发送前还是发送中，每个站点都必须不停的检测信道，边发送边监听；<br>
以太网的发送不确定性:每个站点在发送数据的一段时间内(**争用期/碰撞窗口**---51.2μs),都有可能检测到碰撞，因此不能够保证数据帧一定能够发送成功---以太网规定了最短帧长为64字节(512bit)，对于10M的网络，发送最短帧长需要51.2μs(争用期)；因此凡是小于64字节的帧都是由于冲突而异常中止的无效帧；
